...
def main(_):
  ...
  while True:
    if tf.train.checkpoint_exists(FLAGS.model_dir):
      logging.warning("Flag 1: Loading model checkpoint from {}".format(FLAGS.model_dir))
      ...
      logging.warning("Flag 5: Here we define the predictions function to be passed to the estimator...")
      ...
      if FLAGS.full:
        eval_tag += ".full"
      logging.warning("Flag 6: Entering predictions -> saving INPUT/TARGET/PREDS to {}".format(FLAGS.model_dir))
      params.eval(predictions, eval_dir, global_step, eval_tag,
                  FLAGS.enable_logging)
      logging.warning("Flag 7: Evaluations completed -> saving text metrics to {}".format(FLAGS.model_dir))
      break
    time.sleep(10)
